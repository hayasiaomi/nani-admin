 ...
 */
$.nani.prototype.showMsg = function (text, icon, hideAfter) {
 ...
 * @param hideAfter 隐藏时间
 */
$.nani.prototype.showMsgAndReload = function (text, icon, hideAfter) {
    if (heading == undefined) {
        var heading = "提示";
    }
    $.toast({
 ...
 * @param url 重定向地址
 */
$.nani.prototype.showMsgAndRedirect = function (text, icon, hideAfter, url, isPjax) {
    if (heading == undefined) {
        var heading = "提示";
    }
    $.toast({
 ...
 * @param url url
 */
$.nani.prototype.showMsgAndParentRedirect = function (text, icon, hideAfter, url) {
    if (heading == undefined) {
        var heading = "提示";
    }
    $.toast({
 ...
 * @returns {*} a
 */
$.nani.prototype.formatContent = function (a) {
    a = a.replace(/\r\n/g, '<br/>');
    a = a.replace(/\n/g, '<br/>');
    a = a.replace(/\s/g, ' ');
    return a;
 ...
 * @param title 标题
 */
$.nani.prototype.layerModal = function (url, title) {
    layer.open({
        type: 2,
        title: title,
        shadeClose: true,
...
function saveOptions(option) {
    ...
    $.post('/admin/option/save', param, function (data) {
        if (data.code === 1) {
            nani.showMsg(data.msg, 'success', 1000);
        } else {
            nani.showMsg(data.msg, 'error', 2000);
        }
    }, 'JSON');
}
